<template>
    <div class="picupload">
        ...
        <ul class="img-list">
            <li v-for="(item,index) in uploadArr"><img @click="picShow(index)" :src="item" /><span :data-index="index" @click="delpic"></span></li>
            ...
            methods: {
                ...
                picShow:function (ind) {
                    var that = this,
                        ...
                        pmW = document.documentElement.clientWidth + 'px';
                    // 清除
                    $('.dialog-img').remove();
                    // 放入
                    ...
                    $.each(tmpAry,function (i,n) {
                        ...
                        if (imgNew.width <= imgNew.height) {
                            if (imgNew.width * document.documentElement.clientHeight / imgNew.height > document.documentElement.clientWidth) {
                                sty = 'height:auto;width:' + pmW + ';margin:0 auto';
                            } else {
...
@import '../util/fs.less';
    ...
    .img-list li#upload {
        background: url(//gfs12.gomein.net.cn/T1L_DvBKZ_1RCvBVdK.png) no-repeat center;
        background-size: 0.56rem 0.506667rem;
    }
    .img-list li#upload:before{
                content: "";/* 注意这里为双引号 */
                position: absolute;
                width: 200%;
                height: 200%;
                border: 1px solid #999;
                border-radius: 0.066667rem;/* 也可以设置圆角 */
