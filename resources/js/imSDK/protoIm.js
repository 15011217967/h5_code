...
function Head() {
	...
	this.command = 0;// 命令字
	this.uid = 0; // userId
	this.clientId = 30;//10:ios/11:android/12:wp/20:pc/21:mac/22:ubuntu/23:linux/24:unix/25:ipad/30:Web/40:H5
	...
	this.rspIP = 0; // gateWay ip
	this.rspPort = 0;// gateWay port
	this.traceId = getRandomNum(-2147483648, 2147483647); // 染色id/跟踪id
	this.ack = 0; //回包标记(是否是请求应答包)；0:否、1:是
	this.appId = ""; // 应用id
	this.receiveType = 0; 		//82 发送者类型；0:普通会员；2:系统消息；3:客服消息；4:美号消息；5:机器人消息
	this.senderType  = 0;		//83 接收者类型；0:普通会员；2:系统消息；3:客服消息；4:美号消息；5:机器人消息
    this.sResv = "";//补充 byte[14]
...
function ImMsg() {
	...
	this.attach = []; //附件tional bool origiImg = 14;			//消息类型为图片时，是否有原图/位置
	this.extra =""; //扩展
	...
	this.test = function(){
		...
		case msgTypeConstants.VEDIO:
			if(!isNotEmpty(this.attach)){
				return false;
			}
			if(!(this.attach instanceof Array)){
				return false;
			}
			for(var att in this.attach){
	...
	this.setAttach = function(attach){
		this.attach =attach
		return this;
	};
...
function InitConsultation(){
	 this.uid = null;  			//咨询者id
	 this.groupId = "";  			//群组id，群组ID组成：渠道号_会员ID_4000（渠道=店铺）
	 this.channelId = "";			//渠道商id
	 this.platformId = "";			//平台id
	 this.shopId = "";				//店铺id，无店铺id为自营（我）客服
	 this.entryType = null;		//入口类型，1:店铺首页，2:商品详情,3:订单页
	 this.extra = "";             //扩展，商品信息、订单信息
	this.setUid = function(uid) {
		this.uid = uid;
		return this;
	};
	...
	this.setPlatformId =function(platformId){
	...
	this.setEntryType =function(entryType){
...
//技能组选择提交
// CMD:0x0501
function SelSkillConsultation(){
	this.uid = null;  				//咨询者id
	this.groupId = "";  			//群组id，群组ID组成：渠道号_会员ID_4000（渠道=店铺）
	this.channelId = "";			//渠道商id
	this.platformId = "";			//平台id
	this.shopId = "";				//店铺id，无店铺id为自营（我）客服
	this.skillGroup = null;			//技能组id，售前、售后、支付等
	this.extra = "";				//扩展，商品信息、订单信息
	this.setUid = function(uid) {
		this.uid = uid;
		return this;
	...
	this.setSkillGroup =function(skillGroup){
	...
	this.test = function() {
		return isNotEmpty(this.uid,this.channelId);
	}
}
//客服聊天消息
// CMD:0x0502
function CustomerMsg(){
	...
	this.shopId = "";				//店铺id，无店铺id为自营（我）客服
	this.msgId = "";  			//消息id
	this.msgType = null;  			//消息类型；1:文本、2:语音、3:图片、4:视频、5:位置、6:附件
	this.msgBody = "";  			//消息内容
	this.senderId = null;  			//消息发送者id
	this.senderName = "";  		//消息发送者name
	this.groupType = null;  		//群组类型；1:单聊、2:群聊、3:系统、4:小秘书、5:客服、6：机器人等
	this.sendTime = null; 			//发送时间
	this.msgSeqId = null; 			//消息seq
	this.msgUrl = ""; 			//消息url
	this.attach = null; 		//附件
	this.origiImg = null;			//消息类型为图片时，是否有原图
	this.location =null;	//位置
	this.whetherHide = null; //消息是否隐藏；0:否、1:是（隐藏）,默认显示
	this.receiveUids = null; //显示该条消息的用户id列表；为空时都显示该消息，不为空时列表中的用户显示该消息
	this.msgStatus =null;			//消息状态，0:正常、1:撤回、2:删除、3:阅后焚已读(控制多终端)
	this.groupChatType = null;  	//群组聊天类型；
	//groupType为1（单聊）时【0：普通单聊（默认）、2000：系统消息、3000：客服消息、4000：美号消息、5000：机器人消息】
	//groupType为2（群聊）时【0：好友群聊、1：圈子群聊】
	this.sourceType = null;		//数据来源，0：文章；1：聊天
	this.extra ="";				//扩展
	this.setGroupId = function(groupId) {
		this.groupId = groupId;
		return this;
	...
	this.setOrigiImg = function(origiImg) {
	...
	this.setWhetherHide = function(whetherHide) {
		this.whetherHide = whetherHide;
		return this;
	};
	this.setReceiveUids = function(receiveUids) {
		this.receiveUids = receiveUids;
		return this;
	};
	this.setMsgStatus = function(msgStatus) {
		this.msgStatus = msgStatus;
		return this;
	};
	this.setGroupChatType = function(groupChatType) {
...
function ImCsAttach(){
	this.attachId = "";  			//附件id
	this.attachName = "";  		//附件名称
	this.attachType = null;  		//附件类型
	this.attachUrl = "";  		//附件地址
	this.attachSize = null;  		//附件大小；Kb
	this.width = null;				//图片宽
	this.height = null;				//图片高
	this.attachPlaytime = null;  	//附件播放时长(秒)
	this.attachUploadtime = null; 	//附件上传时间
	this.extra = "";				//扩展
	this.setAttachId = function(attachId) {
		this.attachId = attachId;
		return this;
...
// 位置
function ImCsMsgLocation() {
	this.msgId = ""; 				//消息id
	this.longitude = null;  		//经度
	this.latitude = null;  			//维度
	this.imgUrl = "";  			//截图地址
	this.content = "";  			//地址描述
	this.extra = null;				//扩展
	this.describe = "";			//地址详细描述
	this.setMsgId = function(msgId) {
		this.msgId = msgId;
		return this;
	};
...
function TransferReq(){
	this.reqUid = null;  			//转接发起者id
	this.reqName = "";  			//转接发起者名称
	this.receUid = null;  			//转接接收者id
	this.groupId = "";  			//群组id，群组ID组成：渠道号_会员ID_4000（渠道=店铺）
	this.platformId = "";			//平台id
	this.extra = "";				//扩展
	this.setReqUid = function(reqUid) {
		this.reqUid = reqUid;
		return this;
	};
	this.setReqName = function(reqName) {
		this.reqName = reqName;
		return this;
	};
	this.setReceUid = function(receUid) {
...
//转接接受/拒绝通知
// CMD:0x0505
function TransferResp() {
	this.respUid = null;       			//转接接收者id
	this.respName = "";  			//转接接收者名称
	this.reqUid = null;  			//转接发起者id
	this.groupId = "";  			//群组id，群组ID组成：渠道号_会员ID_4000（渠道=店铺）
	this.platformId = "";			//平台id
	this.sendTime = null; 			//发送时间
	this.extra = "";				//扩展
	this.setRespUid = function(respUid) {
		this.respUid = respUid;
		return this;
	};
	this.setRespName = function(respName) {
...
//旁观者抢答
// CMD:0x0507
function SidelinesAnswer() {
    this.bystander = null;  		//旁观者id
	this.groupId = "";  			//群组id，群组ID组成：渠道号_会员ID_4000（渠道=店铺）
	this.platformId = 3;			//平台id
	this.extra = 4;				//扩展
	this.setBystander = function(bystander) {
...
//旁观者抢答通知，被抢者离开
// CMD:0x0508
function NoticeRobbed() {
	this.bystander = null;  		//旁观/抢麦者id
	this.robbedUid = null;			//被抢麦者id
	this.groupId = "";  			//群组id，群组ID组成：渠道号_会员ID_4000（渠道=店铺）
	this.platformId = "";			//平台id
	this.extra = "";				//扩展
	this.setBystander = function(bystander) {
	...
	this.setRobbedUid = function(robbedUid) {
		this.robbedUid = robbedUid;
		return this;
	};
	this.setGroupId = function(groupId) {
		this.groupId = groupId;
...
function ChangeOnline() {
	this.uid = null;  			//用户id
	this.status = null;			//在线状态，0:离线；1:在线；2:离开
	this.setUid = function(uid) {
		this.uid = uid;
		return this;
	};
...
function SyncEndConsultation(){
	this.groupId = ""; 			//浼氳瘽ID
	this.serviceType = null;		//鏈嶅姟绫诲瀷锛1-鍧愬腑缁撴潫鏈嶅姟
	this.extra = "";				//鎵╁睍
    this.setGroupId = function(groupId){
		this.groupId = groupId;
		return this;
